...
class Tweet(Indexable):
    ...
    def __init__(self, docid, indexable_data, original_data,url,user_id,time_of_visit,relevance,keywords,highlighted_data = None):
    ...
    def __repr__(self):
        return "docid~ %s, tweet~ %s, url~ %s, user_id~ %s, time_of_visit~ %s, relevance~ %s, keywords~ %s" % \
               (self.docid, self.highlighted_data,self.url,self.user_id,self.time_of_visit,self.relevance,self.keywords)
...
class Twitter(object):
    ...
    def load_tweets(self):
        ...
        with open(self.tweets_filename,encoding = "ISO-8859-1") as catalog:
            df=pd.read_csv(self.tweets_filename,encoding = "ISO-8859-1")
            ...
            #for entry in catalog:
                ...
                original_data = entry
                tweet = Tweet(docid, indexable_data, original_data,value['URL'],value['User_Id'],value['Time_Of_Visit'],value['Relevance'],value['Keywords'])
    ...
    def __load_stop_words(self, stop_words_filename):
        ...
        """
        stop_words = {}
        with open(stop_words_filename,encoding="utf8") as stop_words_file:
...
class TwitterDataPreprocessor(object):
    ...
    def preprocess(self, entry):
        ...
        f_entry = self.strip_accents(str(f_entry)).decode('utf-8')
