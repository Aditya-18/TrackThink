...
function download() {
  ...
  var timeOfVisit = sites.timeOfVisit;
  // var siteContent = sites.siteContent;
  var pairs = [];
  for (var site in sitesDict) {
    if (sitesDict.hasOwnProperty(site)){// && timeOfVisit.hasOwnProperty(site)) {
      pairs.push(site + "," + sitesDict[site] + "," + timeOfVisit[site]);
    }
  }
  // for(var site in timeOfVisit) {
  //   pairs.push(site + "," + timeOfVisit[site]);
  // }
  csvContent += pairs.join("\n");
  // window.open(encodeURI(csvContent));
  var filename = "site_details.csv";
